...
router.get("/", (req, res) => {
  Event.find()
    ...
    .catch((err) =>
      res.status(404).json({ eventsNotFound: "No Events found" })
    );
});
...
router.get("/:id", (req, res) => {
  Event.findById(req.params.id)
    ...
    .catch((err) =>
      res
        .status(404)
        .json({ eventNotFound: "Event with that ID does not exist" })
    );
});
...
router.get("/host/:hostId", (req, res) => {
  Event.find({ hostId: req.params.hostId })
    ...
    .catch((err) =>
      res
        .status(404)
        .json({ userEventsNotFound: "This user does not have any events" })
    );
});
